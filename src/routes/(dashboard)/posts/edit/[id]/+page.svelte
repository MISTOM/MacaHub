<script lang="ts">
	...
	// Mock data for existing post (in real app, this would come from an API)
	let existingPost = $state({
		id: page.params.id,
		title: 'Macadamia Pruning Techniques for Better Yield',
		category: 'agronomic-practices',
		contentType: 'article',
		content:
			'<h3>Introduction to Macadamia Pruning</h3><p>Proper pruning is essential for maintaining healthy macadamia trees and maximizing yield. This guide covers the key techniques and timing for effective pruning.</p><h3>Key Pruning Principles</h3><ul><li>Remove dead, diseased, or damaged wood</li><li>Open the canopy for better light penetration</li><li>Maintain tree shape and structure</li><li>Encourage new growth</li></ul><p><strong>Best Time to Prune:</strong> Late winter to early spring, before new growth begins.</p>',
		media: null as File | null,
		tags: ['macadamia', 'pruning', 'yield-optimization', 'tree-care'] as string[],
		status: 'Published',
		author: 'Dr. Jane Mwangi',
		publishedDate: '2024-03-15'
	});

	// Form data - initialize with existing post data
	let formData = $state({
	...
	let hasChanges = $derived(
		formData.title !== existingPost.title ||
			...
			formData.contentType !== existingPost.contentType ||
			formData.content !== existingPost.content ||
			formData.media !== existingPost.media ||
			JSON.stringify([...formData.tags].sort()) !== JSON.stringify([...existingPost.tags].sort())
	);
	...
	async function handleSubmit(isDraft: boolean = false) {
		...
		try {
			...
			console.log('Post updated:', {
				id: page.params.id,
				...formData,
				status: isDraft ? 'Draft' : existingPost.status
			});
	...
	function getContentTypeName(value: string): string {
		return contentTypes.find((type) => type.value === value)?.label || value;
	}
		...
		<div class="mb-8">
			<div class="mb-4 flex items-center space-x-2">
				...
				>
					...
					<span>Back to Extension Services</span>
				</button>
			</div>
			<div class="flex items-start justify-between">
				<div>
					<h1 class="text-primary-dark text-3xl font-bold">Edit Extension Post</h1>
					<p class="text-text/60 mt-2">Update educational content for farmers and stakeholders.</p>
				</div>
				<div class="text-right">
					<div
						class="bg-primary/10 text-primary mb-2 inline-flex items-center rounded-full px-3 py-1 text-sm font-medium"
					>
						<i class="bi-circle-fill text-success mr-2 text-xs"></i>
						{existingPost.status}
					</div>
					<p class="text-text/60 text-sm">
						By {existingPost.author} • {existingPost.publishedDate}
					</p>
				</div>
			</div>
		</div>
		...
		<div class="border-border rounded-lg border bg-white p-8">
			<form onsubmit={() => handleSubmit(false)}>
							...
							{#each formData.tags as tag}
								...
								>
									...
									<button
										...
										class="text-primary/60 hover:text-primary hover:bg-primary/20 flex h-4 w-4 items-center justify-center rounded-full"
				...
				<div class="border-border flex items-center justify-between border-t pt-6">
					...
					<div class="flex items-center space-x-3">
						{#if hasChanges}
							<div class="text-text/60 flex items-center space-x-1 text-sm">
								<i class="bi-circle-fill text-warning text-xs"></i>
